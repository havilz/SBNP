...
export class ExportService {
  ...
  async exportExcel() {
    ...
    const monthYear = new Date().toLocaleString('id-ID', { month: 'long', year: 'numeric' }).toUpperCase();
    ...
    const headers = [
      'NO', 'DSI', 'JENIS / NAMA SBNP', 'POSISI', 'SUMBER TENAGA',
      'PENYEBAB KELAINAN', 'JUMLAH HARI KELAINAN', 'TAHUN PEMBUATAN', 'KONDISI', 'KET'
    ];
    ...
    tableCategories.forEach((cat) => {
        ...
        cat.stations.forEach((station, index) => {
            ...
            row.values = [
                ...
                station.powerSource || '-',
                latestReport ? (ISSUE_STATUS_MAPPING[latestReport.issueStatus]?.label || latestReport.issueStatus) : 'UNKNOWN',
    ...
    for (const cat of categories) {
        ...
        const row = worksheet.getRow(currentRow++);
        row.getCell(1).value = `${cat.name.charAt(0) + cat.name.slice(1).toLowerCase()}`;
    ...
    // Right side summary
    worksheet.getCell(`E${summaryStartRow}`).value = 'Jumlah SBNP bersuar (JSB)';
    worksheet.getCell(`F${summaryStartRow}`).value = `: ${totalUnits - (categories.find(c => c.roman === 'VI')?.stations.length || 0)} unit`;

    worksheet.getCell(`E${summaryStartRow + 1}`).value = 'Jumlah SBNP yang padam (JSBP)';
    worksheet.getCell(`F${summaryStartRow + 1}`).value = `: ${totalOff} unit`;
    
    worksheet.getCell(`E${summaryStartRow + 3}`).value = 'Jumlah Hari Kelainan (JHK)';
    worksheet.getCell(`F${summaryStartRow + 3}`).value = `: ${totalMalfunctionDays} hari`;
  ...
  async exportCsv() {
      ...
      return 'Not implemented for formal format';
  }
}
